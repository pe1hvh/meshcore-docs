...
> [!NOTE]
> In de fork Dutch-MeshCore is `auto` de standaard: daar leidt de firmware de limiet zelf af uit de subband waarop de node is afgestemd, en bepaalt hem opnieuw zodra `set freq` of `set radio` de frequentie verandert. Dat lost het bovenstaande op in de firmware in plaats van in de handleiding. Zie [Forks & varianten](https://domca.nl/#analyse/forks-en-varianten).
