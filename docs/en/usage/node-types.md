...
- **Room Server** — One Room per node, with store-and-forward: messages are held until the recipient is reachable again. The queue holds 32 posts and lives in working memory. Remote administration works with any administrator password; whether your client offers it is another matter — on a T-Deck it takes a registration key, in the smartphone app an unlock.
...
![All five node types in a network: two companion radios, a repeater, a room server, a standalone device and a telemetry node](../../images/en/node-types-1.svg)
