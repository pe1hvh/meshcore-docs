...
component die aan zo'n afspraak voldoet, kan elke andere invulling vervangen
zonder dat de rest van de firmware verandert. Dit hoofdstuk beschrijft de acht
contracten die het ontwerp dragen, en wat elk contract verplicht stelt en wat
het vrijlaat.
...
ingevuld — wie ze overslaat krijgt geen werkende node. Andere hebben een
zinnig standaardantwoord en hoeven alleen te worden ingevuld als de hardware
het aankan.

Die tweedeling is het scharnier van het ontwerp. Een nieuw bord toevoegen
betekent: de verplichte afspraken invullen en de rest laten staan. Een bord
zonder temperatuursensor meldt eenvoudigweg dat er geen temperatuur is, en
niemand hoeft daar rekening mee te houden.
...
De eerste regel is verplicht — elk bord moet zijn batterijspanning kunnen
melden. De drie erna hebben een antwoord voor boards die het niet kunnen: geen
getal, nee, en nul.

![Acht contracten als horizontale balken. Boven elke balk staat wie hem
gebruikt, eronder hoeveel invullingen er bestaan. Radio en scherm hebben er
respectievelijk zes en elf; klok en toevalsbron drie en twee.](../../../images/nl/interfaces-1.svg)

## De acht contracten

| Contract | Verplicht | Invullingen | Gebruikt door |
|---|---|---|---|
...
| Toevalsbron | Willekeurige bytes leveren | 2 | Mesh-logica |
| Gezien-tabel | Vraag of een pakket bekend is, en wissen | 1 | Mesh-logica |
| Pakketvoorraad | Uitgeven, teruggeven, in de rij zetten, ophalen | 1 | Pakketafhandeling |
| Scherm | Aan, uit, wissen, tekenen, doorvoeren | 11 | Applicatie |
| Brug | Pakket aanbieden en ophalen | 2 | Mesh-logica |

De kolom *Invullingen* telt de klassen die het contract in deze commit
daadwerkelijk invullen. Dat er maar één gezien-tabel is en één pakketvoorraad
betekent niet dat het contract overbodig is: het houdt de mesh-logica
onafhankelijk van hoe die tabel is opgebouwd.

Twee getallen verdienen toelichting. Bij *Radio* staat 6 + 1: zes invullingen
lopen via de radiolibrary, en één — ESP-NOW — omzeilt die volledig en zet
WiFi-hardware in als draagvlak. Van die zes worden er vijf gebruikt; de zesde
is aanwezig maar door geen enkel buildtarget geselecteerd.

Bij *Bord* staat 7, niet 4. Drie invullingen zijn gedeeld per platformfamilie
en staan in `src/helpers/`. De vierde familie, RP2040, heeft geen gedeelde
invulling: elk van de vier RP2040-varianten schrijft zijn eigen bordklasse in
zijn eigen variantdirectory. Dat is een asymmetrie in het ontwerp, geen
vergissing van de telling.
...
Dat zijn allemaal keuzes van de invulling.
...
Elf displaytypen vullen dit contract in. Eén ervan doet niets: nodes zonder
scherm krijgen die invulling, zodat de applicatie niet hoeft te vragen of er
een scherm is.

Het contract kent één lek. Een e-ink scherm laat zich niet behandelen als een
LCD — het verversen kost seconden in plaats van milliseconden — en dat verschil
is niet weg te abstraheren. Het contract lost dat op door de applicatie te
laten vragen om wat voor scherm het gaat. Dat is een bewuste concessie; zie
[Ontwerpbeslissingen](decisions.md).

## Wat een contract niet afdwingt

Geen van de contracten legt vast wanneer iets wordt aangeroepen of hoe lang
een aanroep mag duren. Alles draait in één lus, en een invulling die te lang
blijft hangen houdt het hele systeem op. Dat is een afspraak die nergens is
opgeschreven maar wel geldt.

Evenmin dwingt een contract af dat een invulling zich netjes gedraagt bij
